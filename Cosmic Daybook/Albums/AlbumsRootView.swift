...
struct AlbumsRootView: View {
    ...
    private var core: some View {
        ...
        return Group {
            ...
            case .ready:
                #if os(macOS)
                // Not a NavigationSplitView: nested inside the window's split
                // view, macOS 27 hands the inner detail column a leading inset
                // that counts the outer sidebar twice, so every album screen
                // started a full sidebar's width to the right of the albums
                // column. A plain split has no such inset to get wrong.
                HSplitView {
                    AlbumsSidebar(selection: $nav.selection)
                        .frame(minWidth: 200, idealWidth: 225, maxWidth: 320)
                    detail
