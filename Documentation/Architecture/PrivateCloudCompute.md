...
- `AIClientRouter` cascade for "Apple Intelligence (Auto)" is:
  **on-device → Private Cloud Compute**. Claude is never a hidden fallback. There is also a direct
...
- Availability is checked at runtime (`PrivateCloudModelClient.isAvailable`).
  When PCC is unavailable, automatic requests stay on-device. If neither Apple
  model is available, the app reports that Apple Intelligence is unavailable.
