...
| `mastery_candidates` | the read that feeds that batch, and the only thing standing between "presented" and "mastered" that does not require a second trip through the record. `PresentationRecordIndex` gives every enrolled child's given-but-unmarked lessons; a pair is a candidate only with evidence beside the presentation — confirmed ready at capture (3, cited with the day), a closed `.practiceLesson` `CDWorkModel` (3 when its status is `mastered`, else 2), a `CDLessonRecallCheck` with the retained outcome (1) — ranked by summed weight, ties to the lesson given longest ago. `student`, `area`, `group_by` (`student` / `lesson`), `limit` 1–200 (default 40) narrow it. It never writes: it closes with the exact `{"marks":[…]}` JSON for `mark_mastered`, to be sent only once the guide says they assessed those children |
...
| `update_presentation_roster` (write) | the detail view's Save shape — `studentIDs` rewritten in place, `modifiedAt` stamped, confirmed ids pruned; refuses an empty group and presentations already given, and refuses `add_students` naming a withdrawn or transferred child — `remove_students` still takes her off, which is how a departure is cleaned up |
| **Work** | |
...
| `update_work` (write) | `WorkLogService.log`, the one write behind every status change in the app: `status` is the single vocabulary (`active`, `review`, `mastered`, `keepPracticing`, `incomplete`, `complete` = Done); a closing status stamps the completion, writes the `CDWorkCompletionRecord`, marks the day's check-in completed and skips later ones, so the pill leaves the Scheduled strip; `students` restricts the change to those children's linked copies (default every copy, as for check-in moves) and a shared project row refuses a subset; the old `outcome` argument is accepted and folded onto `status`. Also `WorkCompletionService.markCompleted` for `completed_by`; due date, check-in completion, and check-in moves (`move_check_in_from` / `move_check_in_to`, required together — a closed day lands on the next open one like `update_year_plan_entry`; linked copies move together unless `move_check_in_for_this_child_only`, and a sibling whose check-in is already completed keeps its day and is named in the reply) |
...
| `list_orders` / `add_order_items` (write) / `update_order_items` (write) | `CDOrderItem`, the Orders list. Stage (to_request / asked_for / confirmed / received) is read off the item's dates via `CDOrderItem.stage`, as the screen reads it; every change goes through `OrderService`. `add_order_items` skips a link already waiting; `update_order_items` groups items newly marked asked_for in one call into one request. Nothing sends the request email and nothing deletes an item — both stay in the app |
| **Projects** | |
