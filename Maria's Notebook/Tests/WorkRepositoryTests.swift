...
struct WorkRepositoryFetchTests {
    ...
    func fetchWorkModelsReturnsAllWhenNoPredicate() throws {
        ...
        let work1 = WorkModel(title: "Work 1", workType: .practice)
        let work2 = WorkModel(title: "Work 2", workType: .research)
        let work3 = WorkModel(title: "Work 3", workType: .followUp)
    ...
    func fetchWorkModelsRespectsPredicate() throws {
        ...
        let work1 = WorkModel(title: "Practice Work", workType: .practice)
        let work2 = WorkModel(title: "Research Work", workType: .research)
        context.insert(work1)
        context.insert(work2)
        try context.save()

        let repository = WorkRepository(context: context)
        // Fetch all and filter in memory since workTypeRaw is private
        let allWork = repository.fetchWorkModels()
        let fetched = allWork.filter { $0.workType == .practice }

        #expect(fetched.count == 1)
        #expect(fetched[0].workType == .practice)
    }
    ...
    func fetchWorkModelsSortedByCreatedAtDesc() throws {
        ...
        let work1 = WorkModel(title: "Old Work", workType: .practice, createdAt: oldDate)
        let work2 = WorkModel(title: "New Work", workType: .practice, createdAt: newDate)
        context.insert(work1)
        context.insert(work2)
        try context.save()
...
struct WorkRepositoryCreateTests {
    ...
    @Test("createWork sets correct workType for practiceLesson kind")
    ...
    @Test("createWork sets correct workType for followUpAssignment kind")
    ...
    @Test("createWork sets correct workType for research kind")
...
struct WorkRepositoryUpdateTests {
    ...
    func markWorkCompletedSetsNote() throws {
        ...
        #expect(work.notes == "Great progress!")
    }
