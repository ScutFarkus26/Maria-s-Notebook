...
struct ScheduledWorkItemTests {
    ...
    @Test("ScheduledWorkItem id comes from checkIn")
    func idComesFromCheckIn() throws {
        ...
        let item = ScheduledWorkItem(work: work, checkIn: checkIn)

        #expect(item.id == checkIn.id)
    }

    @Test("ScheduledWorkItem stores work and checkIn")
    func storesWorkAndCheckIn() throws {
        let container = try makeContainer()
        let context = ModelContext(container)
        ...
        let checkIn = WorkCheckIn(workID: work.id, date: scheduledDate, status: .scheduled, work: work)
        ...
        #expect(item.checkIn.date == AppCalendar.startOfDay(scheduledDate))
    }
}
