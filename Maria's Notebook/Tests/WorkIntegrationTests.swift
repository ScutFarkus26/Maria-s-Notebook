...
private struct WorkTestContext {
    ...
    func makeWork(title: String = "Test Work", kind: WorkKind = .practiceLesson) -> WorkModel {
        let work = WorkModel(title: title, kind: kind)
        context.insert(work)
        return work
    }
...
struct WorkStudentRelationshipTests {
    ...
    func tracksSingleParticipant() throws {
        ...
        let work = WorkModel(title: "Math Practice", kind: .practiceLesson)
    ...
    func tracksMultipleParticipants() throws {
        ...
        let work = WorkModel(title: "Group Project", kind: .research)
    ...
    func participantCompletionTrackedIndividually() throws {
        ...
        let work = WorkModel(title: "Practice", kind: .practiceLesson)
        work.participants = [
...
struct WorkStatusIntegrationTests {
    ...
    func isOpenFalseWhenStatusComplete() throws {
        let tc = try WorkTestContext.make()
        let work = WorkModel(title: "Practice", kind: .practiceLesson, status: .complete)
        tc.context.insert(work)
...
struct WorkLessonRelationshipTests {
    ...
    func workLinksToLesson() throws {
        ...
        let work = WorkModel(title: "Addition Practice", kind: .practiceLesson, lessonID: lesson.id.uuidString)
        tc.context.insert(work)
        try tc.context.save()
    ...
    func workLinksToStudentLesson() throws {
        ...
        let work = WorkModel(title: "Addition Practice", kind: .practiceLesson, studentLessonID: studentLesson.id)
        tc.context.insert(work)
        try tc.context.save()
...
struct WorkCheckInIntegrationTests {
    ...
    func checkInStatusCanBeUpdated() throws {
        ...
        #expect(checkIn.latestUnifiedNoteText == "Good progress")
    }
    ...
    func checkInCanBeRescheduled() throws {
        ...
        #expect(checkIn.latestUnifiedNoteText == "Moved to next week")
    }
    ...
    func checkInCanBeSkipped() throws {
        ...
        #expect(checkIn.latestUnifiedNoteText == "Student absent")
    }
}
...
struct WorkModelWorkTypeTests {
    ...
    @Test("WorkModel kind getter/setter works")
    func kindGetterSetterWorks() {
        let work = WorkModel(title: "Test", kind: .research)
...
struct WorkPersistenceTests {
    ...
    func handlesNilOptionalFields() throws {
        let tc = try WorkTestContext.make()
        let work = WorkModel(
            title: "Simple Work", kind: .practiceLesson,
