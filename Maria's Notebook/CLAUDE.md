...
- **49 tools covering the whole notebook, reads and writes** (2026-09-07) — the deliberate goal is that anything the guide can see or change in the app is reachable over MCP. `Documentation/Architecture/MCP_SERVER.md` has the full table; the domain files are `MCPNotebookTools+{Reads,Writes,Lessons,Presentations,Meetings,Schedule,ScheduleWrites,Work,WorkWrites,Attendance,Todos,Families,Classroom,Supplies,Projects,Issues,Reference}.swift`.
...
- Where the model has columns the app never adopted, tools follow the app: `CDSupply.minimumThreshold` / `unit` / `isOnOrder` exist in the `.xcdatamodel` but no Swift property declares them and nothing writes them, so `list_supplies` offers a caller-supplied `below` instead of a reorder threshold.
