...
- **74 tools covering the whole notebook, reads and writes** (2026-09-09; `MCPToolRegistryTests` pins the count) — the deliberate goal is that anything the guide can see or change in the app is reachable over MCP. `Documentation/Architecture/MCP_SERVER.md` has the full table; the domain files are `MCPNotebookTools+{Reads,Writes,Lessons,CurriculumSupport,CurriculumReads,CurriculumWrites,CurriculumOrdering,Presentations,PresentationEdits,Meetings,MeetingReads,MeetingScheduling,Schedule,ScheduleWrites,Work,WorkWrites,WorkRemoval,Practice,Attendance,Todos,Families,Communications,Classroom,Jobs,Supplies,Projects,Issues,Reference,Library,Planning,YearPlanWrites,YearPlanClearing,AlbumMarks,SyncStatus}.swift`.
...
- **Backup triggering and AI report drafting are not exposed** because `AutoBackupManager` / `MonthlyReportDraftService` are reached through `AppDependencies` (SwiftUI environment), which a static tool handler cannot resolve. Adding them means a bootstrap-populated service locator, not another tool. `sync_status` only works because `CloudKitSyncStatusService` is a singleton.
