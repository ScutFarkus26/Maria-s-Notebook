...
- **63 tools covering the whole notebook, reads and writes** (2026-09-08) — the deliberate goal is that anything the guide can see or change in the app is reachable over MCP. `Documentation/Architecture/MCP_SERVER.md` has the full table; the domain files are `MCPNotebookTools+{Reads,Writes,Lessons,Presentations,Meetings,MeetingReads,Schedule,ScheduleWrites,Work,WorkWrites,WorkRemoval,Practice,Attendance,Todos,Families,Communications,Classroom,Jobs,Supplies,Projects,Issues,Reference,Library,Planning,AlbumMarks,SyncStatus}.swift`.
