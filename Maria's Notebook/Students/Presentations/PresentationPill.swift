...
struct PresentationPill: View {
    ...
    /// the half the lesson sits in, so a reordered day would otherwise show a
    /// column of identical "9:00 AM" or "2:00 PM" badges (the seconds that
    /// distinguish them are not rendered), and every row written before ordering
    /// existed would read "12:00 AM". None is a time anyone set, so none is
    /// shown — the half itself is what the planner card carries instead.
    private func isDeliberateTime(_ date: Date) -> Bool {
        let calendar = AppCalendar.shared
        let parts = calendar.dateComponents([.hour, .minute, .second], from: date)
        let isMidnight = (parts.hour ?? 0) == 0 && (parts.minute ?? 0) == 0
        let orderingHours = [UIConstants.morningHour, UIConstants.afternoonHour]
        let isOrderingSlot = orderingHours.contains(parts.hour ?? 0) && (parts.minute ?? 0) == 0
