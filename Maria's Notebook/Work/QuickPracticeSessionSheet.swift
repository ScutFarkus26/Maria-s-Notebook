...
struct QuickPracticeSessionSheet: View {
    ...
    private var suggestedPartners: [Student] {
        ...
        // Find co-learners from StudentLesson
        let studentLesson = allStudentLessons.first { sl in
            sl.lessonIDUUID == lessonUUID &&
            sl.studentIDs.contains(studentUUID.uuidString)
        }

        guard let coLearnerIDs = studentLesson?.studentIDs.compactMap({ UUID(uuidString: $0) }) else {
            return []
        }
