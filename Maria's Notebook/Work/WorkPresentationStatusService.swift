...
struct WorkPresentationStatusService {
    ...
    static func findNextPresentationStatus(for work: WorkModel, modelContext: ModelContext) -> PresentationStatus {
        ...
        if let scheduledAssignments = try? modelContext.fetch(scheduledDescriptor) {
        ...
        if let draftAssignments = try? modelContext.fetch(draftDescriptor) {
        ...
        if let studentLessons = try? modelContext.fetch(studentLessonDescriptor) {
