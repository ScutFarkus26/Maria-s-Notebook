...
public final class BackupDiffService {
    ...
    private func analyzeStudentDiff(
        ...
        let currentStudents = (try? modelContext.fetch(FetchDescriptor<Student>())) ?? []
    ...
    private func analyzeLessonDiff(
        ...
        let currentLessons = (try? modelContext.fetch(FetchDescriptor<Lesson>())) ?? []
    ...
    private func analyzeStudentLessonDiff(
        ...
        let current = (try? modelContext.fetch(FetchDescriptor<StudentLesson>())) ?? []
    ...
    private func analyzeNoteDiff(
        ...
        let current = (try? modelContext.fetch(FetchDescriptor<Note>())) ?? []
        let currentIDs = Set(current.map { $0.id })
    ...
    private func analyzeCalendarDiff(
        ...
        let currentNSD = (try? modelContext.fetch(FetchDescriptor<NonSchoolDay>())) ?? []
        let currentOvr = (try? modelContext.fetch(FetchDescriptor<SchoolDayOverride>())) ?? []
    ...
    private func analyzeProjectDiff(
        ...
        let current = (try? modelContext.fetch(FetchDescriptor<Project>())) ?? []
        let currentIDs = Set(current.map { $0.id })
    ...
    private func analyzeAttendanceDiff(
        ...
        let current = (try? modelContext.fetch(FetchDescriptor<AttendanceRecord>())) ?? []
        let currentIDs = Set(current.map { $0.id })
