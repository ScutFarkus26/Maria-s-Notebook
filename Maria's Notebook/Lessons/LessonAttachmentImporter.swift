...
struct LessonAttachmentImporter {
    ...
    func suggestLessons(for fileURL: URL) -> [Lesson] {
        ...
        guard let allLessons = try? modelContext.fetch(descriptor) else {
            return []
        }
    ...
    func getRecentLessons(limit: Int = 5) -> [Lesson] {
        ...
        guard let lessons = try? modelContext.fetch(descriptor) else {
            return []
        }
...
struct LessonAttachmentImportSheet: View {
    ...
    private func loadLessons() {
        ...
        if let lessons = try? modelContext.fetch(descriptor) {
            allLessons = lessons
        }
    }
}
