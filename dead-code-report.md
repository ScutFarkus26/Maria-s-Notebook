...
> **Status update (2026-06-10, final):** Sections **1–4 and 6 are executed**, plus the re-scan's second wave (14 files) and third wave (1 file). The 3 multi-line constants originally skipped are now removed too, along with the helper types that existed only for them (`EntityChange`, `EntitySchemaChanges`, `PayloadField`, `ParshaTopicEntry`). ~~One deliberate exception: `WorkDTO`~~ **Resolved 2026-06-11:** WorkDTO + WorkParticipantDTO deleted (Danny: pre-v11 backups not needed). Separately, the backup round-trip test failures led to a real bug fix: `BackupArchive.isAEAFormat` only accepted "AA01" magic while the writer produces "pbz*" Apple Archives, so every verify/import gate rejected the app's own current backups — fixed by accepting both magics. The new `allModelEntitiesAreRouted` test also surfaced **8 model entities routed to no store** (WorkCycle*, PrepChecklist*, TransitionPlan*, Initiative) — resolved as intentional tombstones: Danny removed those features end-to-end (e.g. commit 5e63e2a0), no code references their classes, and the entity definitions must stay in the model because the CloudKit schema is additive-only. Documented in Phase8PreTests.knownUnroutedEntities. The 3 legacy `*_Previews` structs (section 6) were also left, as previously noted harmless. **Section 5 (uncalled methods) is the only section still open.**
